            ...
            <ul class="select2-results__options"
                ...
                <li
                    *ngFor="let optionValue of optionValuesFiltered;"
                    ...
                    [innerHtml]="optionsDict[optionValue].label">
                    {{optionsDict[optionValue].label}}
                </li>
                <li
                    *ngIf="optionValuesFiltered.length === 0"
                    [ngClass]="[S2_OPTION, S2_MSG]">
                    {{MSG_NOT_FOUND}}
                </li>
            </ul>
        </span>
...
export class SelectDropdownComponent implements AfterViewInit, OnChanges, OnInit {
  ...
  filter(term: string) {
    ...
    for (let i = this.optionValues.length - 1; i >= 0; i--) {
      ...
      let a = label.toUpperCase(); // SIMON; orig --> let a = this.diacriticsService.stripDiacritics(label).toUpperCase();
      let b = term.toUpperCase(); // SIMON; orig --> let b = this.diacriticsService.stripDiacritics(term).toUpperCase();
  ...
  private getOptionClass(optionValue: string): any {
    ...
    result[this.S2_DISABLED] = this.optionsDict[optionValue].disabled;
